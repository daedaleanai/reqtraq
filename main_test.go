...
func TestPreCommitCreateReqGraph(t *testing.T) {
	...
	assert.Equal(t, 20, nLines, "Number of errors is not correct.")
	...
	assert.Contains(t, err.Error(), "Requirement REQ-TEST-SWH-006 in file /testdata/TestPreCommitCreateReqGraph/TEST-137-SRD.lyx has no parents.")
	...
	assert.Contains(t, err.Error(), "Requirement REQ-TEST-SWH-007 in file /testdata/TestPreCommitCreateReqGraph/TEST-137-SRD.lyx has no parents.")
}
